...
# Added by Ferenc
# Variables that would normally come from arguements if ran straight from terminal
...
saved_model_dir = '../tmp/exported_graph'
# saved_model_dir='../Outputs/' + 'test_model' + '/Exported_graph/'
...
def run_final_eval(train_session, module_spec, class_count, image_lists,
  ...
  # Ferenc modified the print to save the results of the validation to a log file
  all_images = 0
  test_nevus_images = 0
  test_melanoma_images = 0
  misclassified_images = 0
  file = open('../Outputs/' + model_name + '/hyperparameters.txt', 'a+')
  file.write('\n\n----------------------------------------------------' + '\n')
  file.write('------------  MISCLASSIFIED TEST IMAGES  -----------' + '\n')
  file.write('----------------------------------------------------' + '\n\n')
  if print_misclassified_test_images:
    tf.logging.info('=== MISCLASSIFIED TEST IMAGES ===')
    for i, test_filename in enumerate(test_filenames):  # to count melanoma / nevus test images
      all_images = all_images + 1
      if '/melanoma/' in test_filename:
        test_melanoma_images = test_melanoma_images + 1
      else:
        test_nevus_images = test_nevus_images + 1
      if predictions[i] != test_ground_truth[i]:
        misclassified_images = misclassified_images + 1
        tf.logging.info('%70s  %s' % (test_filename,
                                      list(image_lists.keys())[predictions[i]]))
        print(str(misclassified_images) + ': ' + '%70s  %s' %
              (test_filename,
               list(image_lists.keys())[predictions[i]]))
        file.write(str(misclassified_images) + ': ' + '%70s  %s' %
                   (test_filename,
                    list(image_lists.keys())[predictions[i]]))
        file.write('\n')
    file.write('\n\n\n')
  ...
  file.write('Number of images used for testing: ' + str(all_images) + '\n')
  file.write('Melanoma images used for testing: ' + str(test_melanoma_images) + '\n')
  file.write('Nevus images used for testing: ' + str(test_nevus_images) + '\n')
  file.close()
...
def train_model():
  ...
  print(image_dir)
  print(output_graph)
  print(intermediate_output_graphs_dir)
  print(output_labels)
  print(summaries_dir)
  print(bottleneck_dir)
